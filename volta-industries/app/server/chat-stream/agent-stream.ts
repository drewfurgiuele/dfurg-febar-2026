...
import {
  buildAgent as buildPlantFloorAgent,
  configureAgentsSdk,
  run as runAgent,
  type AgentContext,
} from '../agent/plantfloor.js';
 ...
 * OpenAI-compatible interface (configured in plantfloor.ts → configureAgentsSdk).
 *
...
export async function streamAgentTurn(args: {
  ...
  modelError: import('../agent/plantfloor.js').ModelErrorDetail | null;
}> {
  ...
  // Captured by the OpenAI fetch shim in plantfloor.ts on any non-2xx
  ...
  const modelError: { current: import('../agent/plantfloor.js').ModelErrorDetail | null } = {
    current: null,
  };
  ...
  try {
    ...
    const agent = buildPlantFloorAgent(ctx);

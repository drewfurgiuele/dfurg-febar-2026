...
type ChatConfig = {
   ...
   * there into the AgentContext used by plantfloor.ts. Replace with
 ...
 * (MAS endpoint OR Genie space — see plantfloor.ts dispatcher).
 *
 * Robustness:
...
export async function handleChatStream(args: {
  ...
  try {
    out = await streamAgentTurn({
      ...
      // (plantfloor.ts `setOpenAIAPI('responses')`). `databricks-gpt-5-4` is the
